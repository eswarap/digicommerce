...
public class TokenService {
    ...
    public TokenService(TokenRepository tokenRepository, VaultService vaultService) {
        this.tokenRepository = tokenRepository;
        this.vaultService = vaultService;
        String secret = vaultService.getSecret("jwt-secret");
    ...
    public String generateToken(String username, String secret) {
        ...
        SecretKey customKey = Keys.hmacShaKeyFor(secret.getBytes());
        Date expiration = new Date(System.currentTimeMillis() + 3600000); // 1 hour
        String jwt = Jwts.builder()
                .subject(username)
                .expiration(expiration)
